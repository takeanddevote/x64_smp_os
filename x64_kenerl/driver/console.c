...
static void set_screen() {
    ...
    outByte(CRT_DATA_REG, ((screen - MEM_BASE) >> 9) & 0xff);
    outByte(CRT_ADDR_REG, CRT_START_ADDR_L);
    outByte(CRT_DATA_REG, ((screen - MEM_BASE) >> 1) & 0xff);
}

static void set_cursor()
{
    outByte(CRT_ADDR_REG, CRT_CURSOR_H);
    outByte(CRT_DATA_REG, ((pos - MEM_BASE) >> 9) & 0xff);
...
void console_clear()
{
    ...
    while ((unsigned long)ptr < MEM_END)
    {
        *ptr++ = 0x0720;
    }
}
